...
function em_turn(p)
	...
	if flr(p.y)==100 and not p.fired then
		local b=em_bullet()
		b:init(em_bullets,p.x,p.y,player.x,player.y,1)
		p.fired = true
	end
	p.dy=p.count
end
...
function enemy(tbl)
	return {
		init = function(self,n,...)
			...
			if n==0 then
				self.sprites,self.func,self.spd,self.hp=
				{16,17,18,19,20},em_straight,1,10
			elseif n==1 then
				self.sprites,self.func,self.count,self.hp=
				{21,22,23},em_turn,1.5,3
			elseif n==2 then
				self.sprites,self.func,self.spd,self.hp=
				{21,22,23},em_wave,1,1
			elseif n==3 then
				self.dir=((player.x>64) and 1 or -1)
				self.sprites,self.func,self.tx,self.count,self.hp=
				{24,25,23},em_slidein_atk,self.x+60*self.dir,-abs(self.x-64),1
			elseif n==4 then
		...
		draw = function(self)
			...
			-- print(self.hp,self.x+8,self.y-4,7)
		end
	}
end
-- set enemies
function set_enemies()
	if g_time%60==0 then
		local pattern=rnd({0,1,2,3})
		if pattern==1 then
			local x=rnd({22,60,98})
			for n=1,3 do
				local e=enemy(enemies)
				e:init(pattern,x,-10*n,1,1)
			end
		elseif pattern==2 then
			local x=rnd({22,60,98})
			for n=1,5 do
				local e=enemy(enemies)
				e:init(pattern,20,-10*n,1,1)
			end
		elseif pattern==3 then
			...
			for n=1,4 do
				local e=enemy(enemies)
				e:init(pattern,x+n*10*dir,20,1,1)
			end
		else
			local e=enemy(enemies)
			e:init(pattern,rnd({22,60,98}),-8,1,1)
		end
	end
end
...

...

